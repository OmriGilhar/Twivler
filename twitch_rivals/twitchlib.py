"""twitchlib.py: internal library to simplify interactions with the Twitch API.
"""
...
ChannelInfo = namedtuple('ChannelInfo', (
    'id name display_name game status logo video_banner'
    ' profile_banner profile_banner_background_color url'
    ' description ')
)
...
def create_game_info(game: dict) -> GameInfo:
    return GameInfo(
        name=game['game']['name'],
        id=game['game']['id'],
        box=game['game']['box'],
...
def get_client_id():
    ...
    # build the path to twitch info json then pars the json and return the
...
def create_stream_info(stream):
    return StreamInfo(
        id=stream['id'], game=stream['game'], preview=stream['preview'])
...
def create_channel_info(channel):
    return ChannelInfo(
        ...
        profile_banner_background_color=channel[
            'profile_banner_background_color'],
        url=channel['url'])


def create_dummy_channel_info():
    return ChannelInfo(id='None',
                       name='None',
                       display_name='None',
                       game='None',
                       status='None',
